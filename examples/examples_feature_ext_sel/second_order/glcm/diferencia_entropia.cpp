...
void usarLibreria(vector<vector<int>> imageData,int rows,int cols){
    ...
    img.guardarValorCSV("SECOND ANGULAR MOMENT",m_dentropy,"/home/user/RESULTADOS/resultadosSecuenciales.csv");
...
void usarLibreriaParalela(vector<vector<int>> imageData,int rows,int cols, int hilos){
    ...
    {
        //3 DIFERENCIA ENTROPIA
        m_dentropy = entropia.f11_dentropy(pMatriz, toneCount, hilos);
        img2.guardarValorCSV("SECOND ANGULAR MOMENT",m_dentropy,"/home/user/RESULTADOS/resultadosParalelo.csv");
    }
