...
void usarLibreria(vector<vector<int>> imageData,int rows,int cols){
    ...
    img.guardarValorCSV("MAXIMO CORRELATION COEFICIENT",m_maxcorr,"/home/user/RESULTADOS/resultadosSecuenciales.csv");
...
void usarLibreriaParalela(vector<vector<int>> imageData,int rows,int cols, int hilos){
    ...
    {
        //9 MAXIMO CORRELATION COEFICIENT
        m_maxcorr = coefi.f14_maxcorr(pMatriz, toneCount, hilos);
        img2.guardarValorCSV("MAXIMO CORRELATION COEFICIENT",m_maxcorr,"/home/user/RESULTADOS/resultadosParalelo.csv");
    }
