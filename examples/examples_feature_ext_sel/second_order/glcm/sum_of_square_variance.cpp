...
void usarLibreria(vector<vector<int>> imageData,int rows,int cols){
    ...
    img.guardarValorCSV("SUM OF SQUARE VARIANCE",m_var,"/home/user/RESULTADOS/resultadosSecuenciales.csv");
...
void usarLibreriaParalela(vector<vector<int>> imageData,int rows,int cols, int hilos){
    ...
    {
        //12 SUM OF SQUARE VARIANCE
        m_var = sqvariance.f4_var(pMatriz, toneCount, hilos);
        img2.guardarValorCSV("SUM OF SQUARE VARIANCE",m_var,"/home/user/RESULTADOS/resultadosParalelo.csv");
    }
