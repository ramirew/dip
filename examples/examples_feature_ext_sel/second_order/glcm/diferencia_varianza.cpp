...
void usarLibreria(vector<vector<int>> imageData,int rows,int cols){
    ...
    img.guardarValorCSV("SECOND ANGULAR MOMENT",m_dvar,"/home/user/RESULTADOS/resultadosSecuenciales.csv");
...
void usarLibreriaParalela(vector<vector<int>> imageData,int rows,int cols, int hilos){
    ...
    {
        //4 DIFERENCIA VARIANZA
        m_dvar = variance.f10_dvar(pMatriz, toneCount, hilos);
        img2.guardarValorCSV("SECOND ANGULAR MOMENT",m_dvar,"/home/user/RESULTADOS/resultadosParalelo.csv");
    }
